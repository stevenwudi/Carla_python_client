...
def make_carla_settings():
    ...
    # Di Wu add Lidar here
    camera3 = sensor.Camera('Lidar')
    camera3.set_image_size(WINDOW_WIDTH, WINDOW_HEIGHT)
    camera3.set_position(200, 0, 140)
...
class CarlaGame(object):
    ...
    def _on_render(self):
        ...
        if self._pred_color is not None:
            surface = pygame.surfarray.make_surface(self._pred_color.swapaxes(0, 1))
            self._display.blit(surface, (gap_x, mini_image_y))

        # If we didn't have segmentation, than we will have the depth for display
        elif self._mini_view_image1 is not None:
